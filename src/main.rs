...
// normal network jitter without regressing startup responsiveness.
const HOME_RESUME_ROW_FETCH_TIMEOUT_SECS: u64 = 6;
const HOME_NEXT_UP_ROW_FETCH_TIMEOUT_SECS: u64 = 6;
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // If auto-login fails, load public users for the login screen.
    {
        ...
        spawn_ui_task(async move {
            ...
            if !authenticated && schedule_saved_token_background_recovery {
                // Keep Home shell visible while saved-token recovery reconnects
                // in the background so startup does not regress back to Login.
                info!(
